        ...
        <ol>     
            @if (!$practises->isEmpty())
                ...
                <li>
                    ...
                    <a class="procedure-name">{{ $practise->name }}</a> <a href="{{ route('field.show',[$activeFarm->id, $practise->field_id]) }}">{{ $practise->field_name }}</a><span class="date">{{\Carbon\Carbon::parse($practise->updated_at)->format('d-m-Y') }}</span>
                </li>
                @endforeach
            @else
